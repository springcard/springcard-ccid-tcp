...
struct networkDevice_MultiSlot_Extension
{
	...
	pthread_mutex_t mutex_reader_access;
};
...
// Function to disable the driver (make pcscd ignore it)
void disable_driver()
{
    setenv("PCSCLITE_FILTER_IGNORE", "1", 1);  // Enable ignoring
    DEBUG_CRITICAL("Driver disabled: PCSCLITE_FILTER_IGNORE=1\n");
}

// Function to enable the driver (allow pcscd to see it again)
void enable_driver()
{
    unsetenv("PCSCLITE_FILTER_IGNORE");  // Remove the variable
    DEBUG_CRITICAL("Driver enabled: PCSCLITE_FILTER_IGNORE unset\n");
}
...
void init_driver(void)
{
	...
	if (driver_setup_is_done == 0)
	{
		...
		for (cpt = 0; cpt < CCID_DRIVER_MAX_READERS; cpt++)
		{
			...
			networkDevice[cpt].multislot_extension = NULL;
			networkDevice[cpt].status = NOT_CONNECTED;
...
static RESPONSECODE connectToReader(unsigned int reader_index)
{
	...
	if (reader_index > 0)
	{
		/*DEBUG_CRITICAL4("gethostbyname: %02X '%s' '%s'",
		strlen( networkDevice[reader_index].reader_ip ),
		networkDevice[reader_index].reader_ip, networkDevice[reader_index-1].reader_ip);*/
...
status_t OpenNetworkByName(unsigned int reader_index, /*@null@*/ char *device)
{
	...
	{
		DEBUG_CRITICAL2("Multi slot readers %02X", reader_index);

		//if (networkDevice[reader_index].multislot_extension == NULL)
		{
			networkDevice[reader_index].multislot_extension = networkDevice[reader_index - 1].multislot_extension;
			//networkDevice[reader_index].multislot_extension->reader_id = reader_index;
			
			networkDevice[reader_index].multislot_extension->status = CONNECTED;
			networkDevice[reader_index].status = CONNECTED;
			DEBUG_CRITICAL3("Multi slot readers %02X %02X ", 
				networkDevice[reader_index].multislot_extension->reader_index,
				networkDevice[reader_index].multislot_extension->reader_id );
		}
	}
	else
	{
		...
		networkDevice[reader_index].ccid.wLcdLayout = (configuration_descriptor[51] << 8) + configuration_descriptor[50];
		...
		DEBUG_INFO2("IFD_bcdDevice " DWORD_X "", (unsigned long)networkDevice[reader_index].ccid.IFD_bcdDevice);
		...
		DEBUG_CRITICAL2("Init  multislot_extension %02X", reader_index);
		networkDevice[reader_index].multislot_extension = Multi_CreateFirstSlot(reader_index);

		/*DEBUG_CRITICAL4("%02X %02X %02X",
						networkDevice[reader_index].multislot_extension->reader_index,
						networkDevice[reader_index].multislot_extension->reader_id,
						networkDevice[reader_index].multislot_extension->slot_id);*/
	}
...
status_t WriteNetwork(unsigned int reader_index, unsigned int length, unsigned char *buffer)
{
	...
	if ( networkDevice[reader_index].multislot_extension == NULL )
	{
		DEBUG_CRITICAL3("WriteNetwork %d %d", reader_index, networkDevice[reader_index].multislot_extension->reader_id);
		disable_driver();
	...
	/* each slot is seen like a reader, slot is still equal to zero*/
	if (reader_index >= networkDevice[reader_index].multislot_extension->reader_id)
	{
		/* networkDevice[reader_index].multislot_extension->reader_id is equal to reader_index */
		/* exemple 0 for reader_index 0 slot 0 */
		/* exemple 0 for reader_index 1 slot 1 */
		/* 0 - 0 for slot 0 of physical reader 1*/
		/* 1 - 0 for slot 1 of physical reader 1*/
		/* 2 - 2 for slot 0 of physical reader 2*/
		buffer[5] = (BYTE)(reader_index - networkDevice[reader_index].multislot_extension->reader_id);
	}
	...
	if (buffer[0] == PC_To_RDR_IccPowerOn)
	{
		...
		if (networkDevice[reader_index].multislot_extension != NULL)
		{
			...
			snprintf(debug_header, 256, "PC_To_RDR_IccPowerOn(%d): %02X %02X %02X %02X %02X %02X %02X %02X %02X %02X %02X ",
					 networkDevice[reader_index].multislot_extension->reader_id, buffer[0], buffer[1], buffer[2], buffer[3], buffer[4], buffer[5], buffer[6], buffer[7], buffer[8], buffer[9], buffer[10]);
		}
	}
	else if (buffer[0] == PC_To_RDR_IccPowerOff)
	{
		cmd_to_send[0] = EP_Bulk_PC_To_RDR;
		snprintf(debug_header, 256, "PC_To_RDR_IccPowerOff(%d): %02X %02X %02X %02X %02X %02X %02X %02X %02X %02X %02X ",
				 networkDevice[reader_index].multislot_extension->reader_id, buffer[0], buffer[1], buffer[2], buffer[3], buffer[4], buffer[5], buffer[6], buffer[7], buffer[8], buffer[9], buffer[10]);
	}
	else if (buffer[0] == PC_To_RDR_GetSlotStatus)
	{
		cmd_to_send[0] = EP_Bulk_PC_To_RDR;
		snprintf(debug_header, 256, "PC_To_RDR_GetSlotStatus(%d): %02X %02X %02X %02X %02X %02X %02X %02X %02X %02X %02X ",
				 networkDevice[reader_index].multislot_extension->reader_id, buffer[0], buffer[1], buffer[2], buffer[3], buffer[4], buffer[5], buffer[6], buffer[7], buffer[8], buffer[9], buffer[10]);
		//show_debug = 1;
	}
	...
	{
		cmd_to_send[0] = EP_Bulk_PC_To_RDR;
		snprintf(debug_header, 256, "PC_To_RDR_XfrBlock(%d): %02X %02X %02X %02X %02X %02X %02X %02X %02X %02X %02X ",
				 networkDevice[reader_index].multislot_extension->reader_id, buffer[0], buffer[1], buffer[2], buffer[3], buffer[4], buffer[5], buffer[6], buffer[7], buffer[8], buffer[9], buffer[10]);
	}
	else if (buffer[0] == PC_To_RDR_Escape)
	{
		cmd_to_send[0] = EP_Bulk_PC_To_RDR;
		snprintf(debug_header, 256, "PC_To_RDR_Escape(%d): %02X %02X %02X %02X %02X %02X %02X %02X %02X %02X %02X %02X",
				 networkDevice[reader_index].multislot_extension->reader_id, buffer[0], buffer[1], buffer[2], buffer[3], buffer[4], buffer[5], buffer[6], buffer[7], buffer[8], buffer[9], buffer[10], buffer[11]);
	}
	else if (buffer[0] == PC_To_RDR_GetParameters)
	{
		cmd_to_send[0] = EP_Bulk_PC_To_RDR;
		snprintf(debug_header, 256, "PC_To_RDR_GetParameters(%d): %02X %02X %02X %02X %02X %02X %02X %02X %02X %02X %02X ",
				 networkDevice[reader_index].multislot_extension->reader_id, buffer[0], buffer[1], buffer[2], buffer[3], buffer[4], buffer[5], buffer[6], buffer[7], buffer[8], buffer[9], buffer[10]);
	}
	else if (buffer[0] == PC_To_RDR_ResetParameters)
	{
		cmd_to_send[0] = EP_Bulk_PC_To_RDR;
		snprintf(debug_header, 256, "PC_To_RDR_ResetParameters(%d): %02X %02X %02X %02X %02X %02X %02X %02X %02X %02X %02X ",
				 networkDevice[reader_index].multislot_extension->reader_id, buffer[0], buffer[1], buffer[2], buffer[3], buffer[4], buffer[5], buffer[6], buffer[7], buffer[8], buffer[9], buffer[10]);
	}
	else if (buffer[0] == PC_To_RDR_SetParameters)
	{
		cmd_to_send[0] = EP_Bulk_PC_To_RDR;
		snprintf(debug_header, 256, "PC_To_RDR_SetParameters(%d): %02X %02X %02X %02X %02X %02X %02X %02X %02X %02X %02X ",
				 networkDevice[reader_index].multislot_extension->reader_id, buffer[0], buffer[1], buffer[2], buffer[3], buffer[4], buffer[5], buffer[6], buffer[7], buffer[8], buffer[9], buffer[10]);
	}
	else if (buffer[0] == PC_To_RDR_IccClock)
	{
		cmd_to_send[0] = EP_Bulk_PC_To_RDR;
		snprintf(debug_header, 256, "PC_To_RDR_IccClock(%d): %02X %02X %02X %02X %02X %02X %02X %02X %02X %02X %02X ",
				 networkDevice[reader_index].multislot_extension->reader_id, buffer[0], buffer[1], buffer[2], buffer[3], buffer[4], buffer[5], buffer[6], buffer[7], buffer[8], buffer[9], buffer[10]);
	}
	else if (buffer[0] == PC_To_RDR_T0APDU)
	{
		cmd_to_send[0] = EP_Bulk_PC_To_RDR;
		snprintf(debug_header, 256, "PC_To_RDR_T0APDU(%d): %02X %02X %02X %02X %02X %02X %02X %02X %02X %02X %02X ",
				 networkDevice[reader_index].multislot_extension->reader_id, buffer[0], buffer[1], buffer[2], buffer[3], buffer[4], buffer[5], buffer[6], buffer[7], buffer[8], buffer[9], buffer[10]);
	}
	else if (buffer[0] == PC_To_RDR_Secure)
	{
		cmd_to_send[0] = EP_Bulk_PC_To_RDR;
		snprintf(debug_header, 256, "PC_To_RDR_Secure(%d): %02X %02X %02X %02X %02X %02X %02X %02X %02X %02X %02X ",
				 networkDevice[reader_index].multislot_extension->reader_id, buffer[0], buffer[1], buffer[2], buffer[3], buffer[4], buffer[5], buffer[6], buffer[7], buffer[8], buffer[9], buffer[10]);
	}
	else if (buffer[0] == PC_To_RDR_Mechanical)
	{
		cmd_to_send[0] = EP_Bulk_PC_To_RDR;
		snprintf(debug_header, 256, "PC_To_RDR_Mechanical(%d): %02X %02X %02X %02X %02X %02X %02X %02X %02X %02X %02X ",
				 networkDevice[reader_index].multislot_extension->reader_id, buffer[0], buffer[1], buffer[2], buffer[3], buffer[4], buffer[5], buffer[6], buffer[7], buffer[8], buffer[9], buffer[10]);
	}
	else if (buffer[0] == PC_To_RDR_Abort)
	{
		cmd_to_send[0] = EP_Bulk_PC_To_RDR;
		snprintf(debug_header, 256, "PC_To_RDR_Abort(%d): %02X %02X %02X %02X %02X %02X %02X %02X %02X %02X %02X ",
				 networkDevice[reader_index].multislot_extension->reader_id, buffer[0], buffer[1], buffer[2], buffer[3], buffer[4], buffer[5], buffer[6], buffer[7], buffer[8], buffer[9], buffer[10]);
	}
	else if (buffer[0] == PC_To_RDR_SetDataRateAndClockFrequency)
	{
		cmd_to_send[0] = EP_Bulk_PC_To_RDR;
		snprintf(debug_header, 256, "PC_To_RDR_SetDataRateAndClockFrequency(%d): %02X %02X %02X %02X %02X %02X %02X %02X %02X %02X %02X ",
				 networkDevice[reader_index].multislot_extension->reader_id, buffer[0], buffer[1], buffer[2], buffer[3], buffer[4], buffer[5], buffer[6], buffer[7], buffer[8], buffer[9], buffer[10]);
	}
	else
	{
	...
	pthread_mutex_lock(&networkDevice[reader_index].multislot_extension->mutex_reader_access);
	rv = write(networkDevice[reader_index].dev_handle, cmd_to_send, length);

	if (rv < 0)
	{
		pthread_mutex_unlock(&networkDevice[reader_index].multislot_extension->mutex_reader_access);
	...
	//if (show_debug == 1)
	{
		DEBUG_CRITICAL3("%s -> WR %d", debug_header, rv);
	}
...
wait_again:
	...
	if ( msExt->signal_done == 0)
	{
		...
		else
		{
			if (rv == ETIMEDOUT)
			{
				...
				if (check_read == 1)
				{
					DEBUG_CRITICAL2("Timeout(%d) - Start", networkDevice[reader_index].multislot_extension->reader_id);
					pthread_mutex_unlock(&msExt->mutex);
					usleep(100);
					goto wait_again;
				}
				else if (check_read > 2)
				{
					DEBUG_CRITICAL2("Timeout(%d) - Stop Ellapsed", networkDevice[reader_index].multislot_extension->reader_id);
	...
	{
		DEBUG_CRITICAL3("???? Condition before wait %d %d", 
			reader_index,
			networkDevice[reader_index].multislot_extension->reader_id);
		memcpy(buffer, msExt->buffer, msExt->buffer_size);
		*length = msExt->buffer_size;
	...
	DEBUG_INFO4("%d Status %d Read %d", reader_index, status, *length);
	//buffer[5] = 0x00; //(BYTE) reader_index;
...
status_t CloseNetwork(unsigned int reader_index)
{
	...
	if (0 == *networkDevice[reader_index].nb_opened_slots)
	{
		...
		if (msExt)
		{
			...
			pthread_mutex_destroy(&msExt->mutex_reader_access);
...
static void *Multi_PollingProc(void *p_ext)
{
	...
	select_again:
			...
			(networkDevice[msExt->reader_index].dev_handle != -1))
		{
			...
			if (rv == -1)
			{
				DEBUG_CRITICAL("select error, force try connection");
				goto read_error;
			}
			...
			{
				//DEBUG_CRITICAL("select no data but try read to check connection");
				/* not go to select again, we try to read from port */
				/* if we got an error, we try the get status command */
				/* it allows to check the manage connect/deconnect status */
			}
		...
		read_again:
			...
			if (rv < 0)
			{
#ifndef _NO_AUTO_STATUS				
				//DEBUG_CRITICAL4("%s read error %d (%s)", networkDevice[msExt->reader_index].reader_ip, errno, strerror(errno));
				/* check status only if no recv started */
				clock_gettime(CLOCK_REALTIME, &ts);
				ts.tv_sec += 1;
				ret = pthread_mutex_timedlock(&networkDevice[msExt->reader_index].multislot_extension->mutex_reader_access, &ts);
				if (ret == 0)
				{					
					DEBUG_CRITICAL2("Check Status no command in progress (%s)",networkDevice[msExt->reader_index].reader_ip);
			
					/* request set status (Keep Alive) */
					rv = write(networkDevice[msExt->reader_index].dev_handle, GET_READER_STATUS, sizeof(GET_READER_STATUS));
					if ( rv <= 0)
					{
						pthread_mutex_unlock(&networkDevice[msExt->reader_index].multislot_extension->mutex_reader_access);
						DEBUG_CRITICAL5("write failed (%s): %d %s %d",
										networkDevice[msExt->reader_index].reader_ip,
										rv, strerror(errno), errno);

						DEBUG_INFO1("STATUS_NO_SUCH_DEVICE");
						networkDevice[msExt->reader_index].status = NOT_CONNECTED;
						for ( int idx = 0; idx < SPRINGCARD_MAX_SLOT; idx++)
						{
							networkDevice[msExt->reader_index].multislot_extension->card_status[idx] = 0xAA;
						}
					...
					{
						/* wait the pong */
						rv = read(networkDevice[msExt->reader_index].dev_handle, ping_buffer, DESC_PING_LEN);
						if ( rv <= 0)
						{
							pthread_mutex_unlock(&networkDevice[msExt->reader_index].multislot_extension->mutex_reader_access);
			...
			else if (rv == 0)
			{
				...
				count_following_no_data++;
				if (count_following_no_data > 10)
				{
					DEBUG_CRITICAL("No data to read, force try connection");
					goto read_error;
				}
				goto select_again;
			}
			count_following_no_data = 0;
		...
		filter_interrupt:
			...
			else if (receive_buffer[0] == EP_Interrupt)
			{
				...
				snprintf(debug_header, 256, "Irq(%d): %X %X %X %X %X %X %X %X %X %X %X %X", msExt->reader_index,
						 ...
						 receive_buffer[8], receive_buffer[9], receive_buffer[10], receive_buffer[11]);

				DEBUG_CRITICAL3("%s RD %d", debug_header, trame_size);

				unsigned char slots_reader_status = receive_buffer[11];
				unsigned char slot_status;
				int idx = 0;

				for (idx = 0; idx < SPRINGCARD_MAX_SLOT; idx++)
				{
					slot_status = (slots_reader_status >> (2 * idx)) & 0x03;
					if (slot_status & 0x01)
					{
						networkDevice[msExt->reader_index].multislot_extension->card_status[idx] &= 0xFD;
						/*DEBUG_CRITICAL3("Card present on %02X %02X",
										networkDevice[msExt->reader_index].multislot_extension->reader_index,
										networkDevice[msExt->reader_index].multislot_extension->reader_id);*/
					}
					else
					{
						networkDevice[msExt->reader_index].multislot_extension->card_status[idx] = 0x02;
						/*DEBUG_CRITICAL3("Card absent on %02X %02X",
										networkDevice[msExt->reader_index].multislot_extension->reader_index,
										networkDevice[msExt->reader_index].multislot_extension->reader_id);*/
					}
					msExt->card_error[idx] = 0x00;
				}
			...
			{
				snprintf(debug_header, 256, "RDR_To_PC_DataBlock(%d) : %X %X %X %X %X %X %X %X %X %X ", msExt->reader_id,
						 receive_buffer[1], receive_buffer[2], receive_buffer[3], receive_buffer[4],
						 receive_buffer[5], receive_buffer[6], receive_buffer[7], receive_buffer[8],
						 receive_buffer[9], receive_buffer[10]);
				if (msExt->power_on_in_progress[(int)receive_buffer[6]] == 1)
				{
					msExt->power_on_in_progress[(int)receive_buffer[6]] = 0;
					if ((int)receive_buffer[6] < SPRINGCARD_MAX_SLOT)
					{
			...
			{
				snprintf(debug_header, 256, "RDR_To_PC_SlotStatus(%d) : %X %X %X %X %X %X %X %X %X %X ", msExt->reader_id,
						 receive_buffer[1], receive_buffer[2], receive_buffer[3], receive_buffer[4],
						 receive_buffer[5], receive_buffer[6], receive_buffer[7], receive_buffer[8],
						 receive_buffer[9], receive_buffer[10]);
				if ((int)receive_buffer[6] < SPRINGCARD_MAX_SLOT)
			...
			{
				snprintf(debug_header, 256, "RDR_To_PC_Parameters(%d): %X %X %X %X %X %X %X %X %X %X ", msExt->reader_id,
						 receive_buffer[1], receive_buffer[2], receive_buffer[3], receive_buffer[4],
						 receive_buffer[5], receive_buffer[6], receive_buffer[7], receive_buffer[8],
						 receive_buffer[9], receive_buffer[10]);
			}
			...
			{
				snprintf(debug_header, 256, "RDR_To_PC_Escape(%d): %X %X %X %X %X %X %X %X %X %X %X", msExt->reader_id,
						 receive_buffer[1], receive_buffer[2], receive_buffer[3], receive_buffer[4],
						 receive_buffer[5], receive_buffer[6], receive_buffer[7], receive_buffer[8],
						 receive_buffer[9], receive_buffer[10], receive_buffer[10]);
			}
			else if (receive_buffer[1] == RDR_To_PC_DataRateAndClockFrequency)
			{
				snprintf(debug_header, 256, "RDR_To_PC_DataRateAndClockFrequency(%d): %X %X %X %X %X %X %X %X %X %X ", msExt->reader_id,
						 receive_buffer[1], receive_buffer[2], receive_buffer[3], receive_buffer[4],
						 receive_buffer[5], receive_buffer[6], receive_buffer[7], receive_buffer[8],
						 receive_buffer[9], receive_buffer[10]);
			}
			...
			{
				snprintf(debug_header, 256, "RDR_to_PC_NotifySlotChange(%d): %X %X %X %X %X %X %X %X %X %X ", msExt->reader_id,
						 receive_buffer[1], receive_buffer[2], receive_buffer[3], receive_buffer[4],
						 receive_buffer[5], receive_buffer[6], receive_buffer[7], receive_buffer[8],
						 receive_buffer[9], receive_buffer[10]);
			}
			...
			{
				snprintf(debug_header, 256, "RDR_to_PC_AnswerToUnsupportedMessage(%d): %X %X %X %X %X %X %X %X %X %X ", msExt->reader_id,
						 receive_buffer[1], receive_buffer[2], receive_buffer[3], receive_buffer[4],
						 receive_buffer[5], receive_buffer[6], receive_buffer[7], receive_buffer[8],
						 receive_buffer[9], receive_buffer[10]);
			}
...
RESPONSECODE CmdNetworkGetSlotStatus(unsigned int reader_index, unsigned char buffer[])
{
	...
	// DEBUG_INFO5("CmdNetworkGetSlotStatus %02d -> %X %X %X", reader_index, buffer[5], buffer[6], buffer[7]);
	...
	if (reader_index >= networkDevice[reader_index].multislot_extension->reader_id)
	{
		buffer[7] = networkDevice[reader_index].multislot_extension->card_status[reader_index - networkDevice[reader_index].multislot_extension->reader_id];
	}
	else
	{
		DEBUG_CRITICAL3("CmdNetworkGetSlotStatus %02X %02X", reader_index, networkDevice[reader_index].multislot_extension->reader_id);
	}
...
static struct networkDevice_MultiSlot_Extension *Multi_CreateFirstSlot(int reader_index)
{
	...
	pthread_mutex_init(&msExt->mutex_reader_access, NULL);
